...
## 按位置变化的高斯模糊（2026-09-07）

用户明确否定固定半径模糊淡出和厚重底色。替换本轮顶区实现：每个输出位置按纵坐标计算高斯sigma，上强下弱直至0；不改变导航与内容对齐。先用垂直黑白边缘的扩散宽度测试区分真实半径变化与透明度混合；Impeller路径使用两次一维高斯卷积，不支持shader过滤的后端采用不同sigma的局部高斯过滤降级。去掉渐变擦除与厚重surface遮罩，截图复核滚动时的文字、彩色卡片及明暗模式。无新依赖，不修改并行同步工作，不自动发布。

本次修正结果：

- 顶部改为按纵坐标递减 sigma 的二维高斯卷积：先纵向、再横向，末尾16dp严格保持原图。删除固定模糊的透明度擦除与背景染色；标题、导航和正文仍沿用既有对齐规则。
- 新增 `shaders/tablet_variable_gaussian.frag` 并在 `pubspec.yaml` 注册。每个物理像素都进入离散高斯核，相邻采样配合线性纹理过滤合并，避免稀疏采样导致文字重影和细条纹混叠。旧Skia后端采用32段不同sigma的近似降级；shader加载失败也可降级，加载中保持透明。
- 像素回归覆盖边缘扩散宽度、双轴棋盘、高频细条纹、DPR 1/2/3及关闭效果，Impeller与Skia各5项通过。完整壳层3项通过，包含对齐、横竖屏、窄窗、大字号及滚动截图。全项目静态分析无问题。
- 本机M2的Metal离屏诊断（1194×208dp，含像素回读）中，DPR2关闭/开启中位耗时为6.11/12.33ms，DPR3为10.06/28.81ms；这不是屏幕帧率或实体平板性能证明。默认Impeller widget tester使用SwiftShader软件Vulkan，其耗时不能当作硬件GPU耗时。实体iPad与Android平板的滚动帧率仍待验证。
- 线性采样配置依赖当前Flutter引擎替换输入纹理时保留sampler过滤描述；升级Flutter时必须保留高频条纹像素回归。没有增加依赖，版本仍为2.6.6；本次修正未发布。
