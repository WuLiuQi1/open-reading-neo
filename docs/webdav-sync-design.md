...
> 2026-09-08 更新：本文记录元数据与书籍上传/恢复协议。可编辑 TXT 已有独立的双向同步实现（`MutableTxtSyncService`），使用条件写入、冲突保留及可选 v3 分块传输；与普通书籍原文件上传分别管理。

## 2026-09-08 新增功能同步

- 阅读器设置补齐自动翻页的横向/纵向模式、四种模式时长与快捷入口显示，共七个独立记录。复用当前时长默认值与 5–120 秒边界；旧的统一间隔仅作为缺失新设置时的迁移输入，不覆盖已保存的独立速度。同步恢复不启动自动翻页。
- `book_source_groups` 数据集承载书源分组名称、空分组及顺序，沿用“书源”同步开关。分组名称包括用于本机私密/兼容书源的组名，但不包含这些书源的身份、规则、账号或凭据。书源成员关系仍在各书源记录中，恢复分组目录不删除本机书源或成员关系。
- 保持元数据协议版本不变。旧客户端保留未知记录，新客户端在范围开启后回放；自动翻页按字段合并，分组目录作为一个有序列表按既有 HLC 规则决胜，同一目录的并发编辑不会逐项合并。
- 新设备尚未保存的自动翻页默认偏好及首次空分组目录不生成上传记录，避免首次扫描的较新时间戳覆盖云端数据。分组目录在书源成员关系落地后恢复，避免批次到达顺序使已删除分组复活；损坏的本机分组存储阻止同步，不按空目录上传。
- 本轮未连接真实 WebDAV 服务或进行双设备实机联调。

实现入口：`adapters/metadata_sync_adapters.dart`、`adapters/book_source_groups_sync_adapter.dart`、`sync_dataset_catalog.dart`（均在 `lib/services/sync/` 下），以及 `lib/book_sources/services/book_source_registry.dart`。同步范围页面与中、繁中、英、日文案同步更新。分组写入复用现有注册表队列；排序前复制列表，修复空注册表创建分组时修改不可变列表的异常，未新增依赖。

验证：15 个测试文件各自独立进程运行，共 140 项通过，覆盖自动翻页设置、分组目录、元数据、笔记、连接能力、配置与页面；窄屏浅色及暗色 1.6 倍文字渲染已检查。`flutter analyze --no-pub` 与 `git diff --check` 通过。尚未打包或发布。
